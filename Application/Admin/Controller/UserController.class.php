    ...
    public function login(){
        ...
        if($_req){
            if($_req['username'] == 'admin' && $_req['password'] == 'admin2016'){
                ...
                echo_script("window.location.href='".U('/Admin/Index/index')."'");
            }else{
    ...
    public function index(){
        ...
        if($_req['username']){
            ...
            if($userInfo === false){
                ...
                echo_script("window.location.href='".U('/Admin/User/index')."'");
            }
            $this->assign('user_info',$userInfo ? array($userInfo) : '');
            $this->display();
        }else{
            $userInfo       = D('User','Logic')->getUserList($_GET['p']);
            $Page       = new \Think\Page(D('UserInfo')->count(),25);
            $show       = $Page->show();
